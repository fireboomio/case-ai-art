...
## 1. 项目需求
~~https://share.lanhuapp.com/#/invite?sid=lXKFUBEa~~

https://lanhuapp.com/link/#/invite?sid=lX0tcrqd
...
https://ide.code.fun/projects/63aafc28d109e600121b3472/pages
...
export default async function postAuthentication(hook: AuthenticationHookRequest) : Promise<void>{
  if (hook.user?.userId && hook.user.providerId !== 'admin') {
    const { provider, providerId, userId, picture, nickName } = hook.user
...
使用 mutatingPreResolve钩子设置每种积分方式的分值
...
使用customResolve钩子判断今日积分增加次数是否用完
...
使用批量新建创建接口GetOneAppUser，勾选AppUser的选项（如下图）

![](https://cdn.nlark.com/yuque/0/2023/png/26463209/1678086006540-734c3ddb-0895-46b8-b9df-244b86a9cab5.png?x-oss-process=image%2Fresize%2Cw_1500%2Climit_0)

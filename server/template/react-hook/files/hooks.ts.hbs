...
import { RequestOptions, MutateRequestOptions, SubscriptionRequestOptions, Response} from "fireboom-wundersdk"
{{#if hasImports}}import { {{imports}} } from "./models";{{/if}}

export const useFireboom = () => {
    const ctx = useContext(WunderGraphContext)
    if (ctx === undefined) {
        throw new Error("WunderGraphContext missing, make sure to put FireboomProvider at the root of your app")
    }
    return {
        client: ctx.client,
...
{{#if hookHasQueries}}
    const Query = <R extends {}, I extends {}>(
        ...
        const {initialized,{{#if hasAuthProviders}}user,{{/if}}onWindowFocus,refetchMountedQueries,queryCache} = useFireboom();
...
{{#if  hookHasMutations}}
    const Mutation = <R extends {}, I extends {}>(
        ...
        const { {{#if hasAuthProviders}}user,{{/if}}setRefetchMountedQueries} = useFireboom();
...
{{#if  hookHasSubscriptionsOrLiveQueries }}{{#unless reactNative}}
const Subscription = <R, I>(
    ...
    const { {{#if hasAuthProviders}}user,{{/if}} initialized, refetchMountedQueries } = useFireboom();
...
{{#if  hookHasSubscriptions}}{{#unless reactNative}}
export const useSubscription = {
{{#each hookSubscriptions}}
...
{{#if  hookHasLiveQueries}}{{#unless reactNative}}
    export const useLiveQuery = {
    {{#each hookLiveQueries }}
        {{name}}: (options{{#unless hasInput}}?{{/unless}}: SubscriptionRequestOptions{{#if hasInput}}<{{name}}Input>{{/if}}) => {
            const {client} = useFireboom();
